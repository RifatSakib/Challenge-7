...
function App() {
  const [selectItems, setSelectItems] = useState([]); // this is for new player 

  const [autoPrice, setAutoPrice] = useState(0); // this is for auto-price
  const incrementAmount = autoPrice;
  // console.log (autoPrice);

  const [validation, setValidation] = useState(0); // this is for player price
  console.log (validation);
  const decrementAmount = validation
  

  const [removecard, setRemoveCard] = useState(0); // this is for

  const handleRemoveCard = (id)=>{
    console.log(id)

    const remainingCards = selectItems.filter(player => player.playerId !== id);
    console.log(remainingCards);
    setSelectItems(remainingCards);

  }


  const [togglebtn, setTogglebtn] = useState(true); // this is for toggle button
...
const handleValidation = (payment)=>{
  ...
  return (
    <>
      ...
      {
        togglebtn ? <Card handleAddToSelect={handleAddToSelect} selectItems={selectItems} handleValidation={handleValidation}  autoPrice={autoPrice}  validation={validation}></Card> : <SelectedPlayers selectItems={selectItems}  handleToogleBtn= {handleToogleBtn} handleRemoveCard={handleRemoveCard} ></SelectedPlayers>
